...
#letter size 2.5 deg
SOAms = 400 #350 #250 #360 #133 #170 #133 #Battelli, Agosta, Goodbourn, Holcombe mostly using 133
...
letterDurMs = 250#200 #300 #130 #80 #23.6  in Martini E2 and E1b (actually he used 22.2 but that's because he had a crazy refresh rate of 90 Hz)
...
for cueSerialPos in cueSerialPositions:
  for leftRight in [False,True]:
      for wordEcc in [5]:
       for hemifield in [1]: #only does something in the vertical configuration
...
if doStaircase:
    ...
    for condition in (firstCondition,secondCondition):
        ...
        while nDoneMain < trials.nTotal and expStop==False: #MAIN EXPERIMENT LOOP
            ...
            numCharsInResponse = 1
            if thisTrial['leftRight']:
                responseOrder = [1]
            else: responseOrder = [0]
            x = 1.5 * thisTrial['wordEccentricity']*(thisTrial['leftRight']*2-1) #put it 3 times farther out than stimulus, so participant is sure which is left and which right
            ...
            expStop, passThisTrial, response, responseAutopilot = stringResponseKReditPython3.collectStringResponse(
            ...
            if not expStop:
                ...
                print(thisTrial['leftRight'],'\t', end='', file=dataFile)
                ...
                correct,approxCorrect,responsePosRelative = (
                        handleAndScoreResponse(passThisTrial,response,responseAutopilot,task,sequenceStream,thisTrial['cueSerialPos'],correctAnswerIdx,wordList) )
                ...
                print('correct=',correct,' approxCorrect=',approxCorrect,' responsePosRelative=', responsePosRelative)
                numTrialsCorrect += correct #so count -1 as 0
                numTrialsApproxCorrect += approxCorrect
