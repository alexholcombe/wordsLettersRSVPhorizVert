...
```{r, echo=FALSE}
# doubleGaussian = function(x, mean,sdLeft,sdRight) {
#   print(x)
#   if (x<mean) {
#     y = dnorm(x,mean,sdLeft)
#   } else {
#     y = dnorm(x,mean,sdRight)
#   }
#   y
# }
...
doubleGaussian = function(x, mean,sdLeft,sdRight) {
  ...
  if (x<mean) { 
    ...
    dnorm(x,mean,sdLeft) / leftPeakHeight
  } else {
    rightPeakHeight = dnorm(mean,mean,sdRight)
    dnorm(x,mean,sdRight) / rightPeakHeight
  }
}
...
library('ggplot2')
ggplot(df, aes(x=x,y=density)) + geom_line() + xlab("x") 
```
