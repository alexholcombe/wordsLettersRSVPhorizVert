...
Plot the letters prior, a DoubleGaussian distribution with our mean observed effect (as a Cohen's d), `r modalEffectSize` and sigma left of mode=`r sigmaLeft` and sigma right of mode=`r sigmaRight`.
...
plot(dDomain,priorWords(dDomain),typ='n',xlab="Top bias (Cohen's d)",ylab="Density",ylim=c(0,1.4*maxAlt),main="Words and letters top bias prior")
arrows(0,0,0,1,col='darkblue',lwd=2)
lines(dDomain,priorWords(dDomain),col='darkgreen',lwd=3)
lines(dDomain,priorLtrs(dDomain),col='green',lwd=2)

legend("topright",legend=c("Null","AlternativeLtrs","AlternativeWords","Previous findings"),col=c('darkblue','green','darkgreen','black'),lwd=2)
...
1. Whether the top bias depends on whether the streams are presented in the left or right hemifield. 
